...
@interface T367ViewController ()

@end

@implementation T367ViewController
...
- (void)loadFirstViewWithScrollView:(UIScrollView *)scrollView
{
    ...
    ColorLabel *l2 = [[ColorLabel alloc] initWithPosition:ccp(_topOffset+10, _topOffset+10) text:LocStr(@"  已选      个 (可选7-15个)") color:NAVITITLECOLOR];
    ...
    // 画球
    UIView *c1 = [self drawBallView:36 withPosition:ccp(_topOffset+5,l2.bottom + 5.0) redOrBlue:kColorRed callback:@selector(zixuanBallSelected:) selectBallArray:zixuan_Array];
...
{
    CGRect frame = CGRectMake(_topOffset,35.0,mScreenWidth-20,20);
    ...
    UIView *c1 = [self drawBallView:36 withPosition:ccp(_topOffset+5,l2.bottom + 5.0) redOrBlue:kColorRed callback:@selector(dantuo_dan_selected:) selectBallArray:_danqu];
    ...
    UIView *c2 = [self drawBallView:36 withPosition:ccp(_topOffset+5,l4.bottom + 5.0) redOrBlue:kColorRed callback:@selector(dantuo_tuo_selected:) selectBallArray:_tuoqu];
...
- (void)loadThirdViewWithScrollView:(UIScrollView *)scrollView
{
    ...
    for (int row = 0; row < self.selectPickerIndex; row ++) {
        ...
        NSArray *red =  getUnRepeatRadomNumber(7, 36);
...
-(BetResult *)randomOneResult
{
    ...
    [result setPlay_code:GET_INT(pT367_Danshi)];
    return result;
}
    ...
    if (_selectIdx == 0) {
        ...
        if (_numberOfBet > 1) {
            ...
            _betString = [_cArray getNumberStringByFormat:@"%02d"
                                           joinedByString:@""];
            _subCode = pT367_Danshi;
        }
        
    } else if (_selectIdx == 1) { // 胆托（胆码区号码#拖码区号码）
        if ([_danqu count]+[_tuoqu count] < 8) {
            [SVProgressHUD showInfoWithStatus:@"胆+拖必须大于7个"];
            return nil;
        }
        
        _subCode = pT367_Dantuo;
        _amount = _numberOfBet;
...
//            [self loadThirdViewByExistsBet:mArray];
    ...
    } else if (lottery_code == pT367_Fushi) {
        _selectIdx = 0;
        ((UIRadioControl*)[self.titleView viewWithTag:RADIOCONTROL_TAG]).selectedIndex = 0;
        ...
        [self loadFirstViewWithScrollView:nil];
        [self checkTotal];
    } else if (lottery_code == pT367_Dantuo) {
        _selectIdx = 1;
