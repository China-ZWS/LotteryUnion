...
@interface T317ViewController ()

@end

@implementation T317ViewController
...
-(void)_initBaseSubViews
{
    ...
    //
    _selectIdx = radio.selectedIndex;
...
- (void)loadFirstViewWithScrollView:(UIScrollView *)scrollView
{
    ...
    ColorLabel *l2 = [[ColorLabel alloc] initWithPosition:ccp(_topOffset, _topOffset+10) text:LocStr(@"  已选      个 (可选7-15个)") color:NAVITITLECOLOR];
    [_fristContainView addSubview:l2];
    
    //TODO:摇一摇按钮
    UIButton *conShake = [[UIButton alloc]initWithFrame:mRectMake(mScreenWidth-90, l2.origin.y-8, 90, l2.height+16)];
    ...
    // 画球
    UIView *c1 = [self drawBallView:31 withPosition:ccp(_topOffset+5,l2.bottom + 5.0) redOrBlue:kColorRed callback:@selector(zixuanBallSelected:) selectBallArray:zixuan_Array];
...
{
    CGRect frame = CGRectMake(_topOffset+10,35.0,mScreenWidth-20,20);
    ...
    ColorLabel *l2 = [[ColorLabel alloc] initWithPosition:ccp(l1.right, _topOffset) text:LocStr(@"  已选      个 (可选1-6个)") color:NAVITITLECOLOR];
    ...
    UIView *c1 = [self drawBallView:31 withPosition:ccp(_topOffset+5,l2.bottom + 5.0) redOrBlue:kColorRed callback:@selector(dantuo_dan_selected:) selectBallArray:_danqu];
    ...
    ColorLabel *l4 = [[ColorLabel alloc] initWithPosition:ccp(l3.right, c1.bottom + 10.0) text:LocStr(@"  已选      个 (胆+拖>＝8个)") color:NAVITITLECOLOR];
    ...
    UIView *c2 = [self drawBallView:31 withPosition:ccp(_topOffset+5,l4.bottom + 5.0) redOrBlue:kColorRed callback:@selector(dantuo_tuo_selected:) selectBallArray:_tuoqu];
...
- (void)loadThirdViewWithScrollView:(UIScrollView *)scrollView
{
    ...
    for (int row = 0; row < self.selectPickerIndex; row ++) {
        ...
        // 两个参数分别表示为（几注，在多少个数里面选）
        NSArray *red =  getUnRepeatRadomNumber(7, 31);
...
-(BetResult *)randomOneResult
{
    ...
    NSArray *red =  getUnRepeatRadomNumber(7, 31);
    [ball addObjectsFromArray:[red sort]];
    [result setNumbers:[ball componentsJoinedByFormat:@"%02d"]]; // 设置号码
    [result setPlay_code:GET_INT(pT317_Danshi)];
    return result;
}
...
- (void)getOneResultWithClick
{
    ...
    for (int i = 0; i < 7; i++) {// 随机出五个
        int m = randomValueBetween(1, 31);
...
- (void)zixuanBallSelected:(BallView *)button
{
    ...
    zixuan_Label.text = [NSString stringWithFormat:@"%lu",(long)[zixuan_Array count]];
    // 计算注数
    [self checkTotal];
}
...
- (void)dantuo_dan_selected:(BallView *)button
{
    ...
    if (isRepeat && !button.selected) {
        ...
        return;
    } else if ([_danqu count] >= 6 && !button.selected) {
        [SVProgressHUD showInfoWithStatus:@"胆码区最多6个"];
        return;
    }
    
    // 选号时震动
    [super vibrateWhenSelectBall];
    button.selected = !button.selected; // 改变状态
    [button isSelectedWithChange];
    ...
    // 赋值
    cFront0.text = [NSString stringWithFormat:@"%lu",(unsigned long)[_danqu count]];
...
- (void)dantuo_tuo_selected:(BallView *)button
{
    ...
    cFront1.text = [NSString stringWithFormat:@"%lu",(unsigned long)[_tuoqu count]];
...
- (void)checkTotal
{
    _numberOfBet = 0;      // 注数
    if(_selectIdx == 0) {  // 自选时
        if ([zixuan_Array count] >= 7) {
            // 计算注数
            _numberOfBet = combination((int)[zixuan_Array count],7);
            
        }
    } else if(_selectIdx == 1) { // 胆托时
        int danCount=(int)_danqu.count;
        int tuoCount=(int)_tuoqu.count;
        if (danCount+tuoCount >= 8 && danCount >= 1) {
                _numberOfBet = combination(tuoCount, 7-danCount);
        }
    }
    
    // 赋值
    [_numberLabel setText:[NSString stringWithFormat:@"%d 注",(int)_numberOfBet]];
    [_totalLabel setText:[NSString stringWithFormat:@"共 %d 元",(int)_numberOfBet*2]];
}
...
//            [mStr appendString:@"#"];
            ...
            _betString = mStr;
            _subCode = pT317_Fushi;
        } else {
            NSMutableArray *_cArray = [NSMutableArray array];
            [_cArray addObjectsFromArray:[zixuan_Array sort]];
//            [_cArray addObjectsFromArray:[_qianqu1 sort]];
            
            _betString = [_cArray getNumberStringByFormat:@"%02d"
                                           joinedByString:@""];
            _subCode = pT317_Danshi;
        }
        
    } else if (_selectIdx == 1) { // 胆托（胆码区号码#拖码区号码）
        if ([_danqu count]+[_tuoqu count] < 8) {
            [SVProgressHUD showInfoWithStatus:@"胆+拖必须大于8个"];
            return nil;
        }
        
        _subCode = pT317_Dantuo;
        _amount = (int)_numberOfBet;
