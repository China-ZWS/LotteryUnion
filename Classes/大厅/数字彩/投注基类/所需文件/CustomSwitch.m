...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        [self _initWithButton];
    }
    return self;
}

-(void)_initWithButton
{
    _onButton = [[UIButton alloc] initWithFrame:CGRectMake(self.frame.origin.x+20, 0, self.frame.size.width - _onButton.frame.origin.x*2, self.frame.size.height)];
    _onButton.tag = 1;
    _offButton = [[UIButton alloc] initWithFrame:CGRectMake(self.frame.origin.x+20, 0, self.frame.size.width - _offButton.frame.origin.x*2, self.frame.size.height)];
    _offButton.tag = 2;
...
-(id)initWithOnImage:(UIImage*)onImage offImage:(UIImage*)offImage arrange:(CustomSwitchArrange)arrange
{
    ...
    if (_arrange == CustomSwitchArrangeONLeftOFFRight) {
        ...
        _offButton.frame = CGRectMake(_onImage.size.width/2-3, 0, _offImage.size.width, _offImage.size.height);
    }else{
...
- (void)drawRect:(CGRect)rect
{
    if (!first) {
        ...
        _leftRect = CGRectMake(-(_onImage.size.width/2-6), 0, _onImage.size.width, _onImage.size.height);
        _middleRect = CGRectMake(0, 0, _onImage.size.width, _onImage.size.height-2);
        ...
        _minusTranslate = _onImage.size.width/2+2;
        _currentTranslationX = 0;
        if (_arrange == CustomSwitchArrangeONLeftOFFRight) {
